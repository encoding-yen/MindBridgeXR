...
void taskHW(void*) {
  ...
  for (;;) {
    ...
    if ((int32_t)(now - lastCtrl) >= (int32_t)HW_PERIOD_MS) {
      ...
      // Enqueue a sample for networking (0 altitude if the ToF reading was
      // invalid this cycle -- server-side can treat 0 as "no reading" if
      // needed, or we can add a separate valid flag later)
      PoseSample s{pitch, yaw, roll, altitudeValid ? altitude : 0};
      enqueueSample(s);
    }
  }
}
...
void taskNet(void*) {
  ...
  for (;;) {
    ...
    int code = postPoseSample(s);
    if (!(code > 0 && code / 100 == 2)) {
      Serial.println("[NET] re-queueing sample.");
      enqueueSample(s);
